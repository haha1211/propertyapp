...
function pyeongToSquareMeters(value?: number) {
  return value === undefined ? undefined : Number((value * PYEONG_TO_M2).toFixed(2));
}
...
export const buyerPreferencesSchema = z
  .object({
    ...
    minAreaPyeong: optionalNumber,
    maxAreaPyeong: optionalNumber,
  ...
  .superRefine((value, ctx) => {
    ...
    if (value.minAreaPyeong && value.maxAreaPyeong && value.minAreaPyeong > value.maxAreaPyeong) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["maxAreaPyeong"],
        message: "최대 평수는 최소 평수보다 커야 합니다."
      });
    }
  ...
  .transform(({ minAreaPyeong, maxAreaPyeong, ...value }) => ({
    ...value,
    minAreaM2: pyeongToSquareMeters(minAreaPyeong),
    maxAreaM2: pyeongToSquareMeters(maxAreaPyeong)
  }));
